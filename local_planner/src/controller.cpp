...
void publishCheckAreaFromGlobalVector() {
    if (!gridReady) return;
...
void Compute(){
    ...
    publishCheckAreaFromGlobalVector();
    ...
    if (all_safe){
        ...
        ROS_INFO("CASE 'ALL SAFE'");

        vizSelPath(selIDX);

        int pathlen = static_cast<int>(m_TentaclePath[selIDX].size());
        int halflen = pathlen / 2; // 이게맞나ㅠ
        steering = std::atan2(m_TentaclePath[selIDX][halflen].y, m_TentaclePath[selIDX][halflen].x);

        steering = - int(2000 * steering / 0.4922); // 왼쪽이 - steering이라 -가 붙어야 함
        ...
        auto max_iter = std::max_element(total_score.begin(), total_score.end());
        selIDX = std::distance(total_score.begin(), max_iter);
        ROS_INFO("CASE 'Else' ");
        ...
        int pathlen = static_cast<int>(m_TentaclePath[selIDX].size());
        int halflen = pathlen / 2; // 이게맞나ㅠ
        steering = std::atan2(m_TentaclePath[selIDX][halflen].y, m_TentaclePath[selIDX][halflen].x); // radian
...
int main(int argc, char** argv){
    ...
    check_area_marker_pub = nh_.advertise<visualization_msgs::Marker>("check_area_marker", 1);
